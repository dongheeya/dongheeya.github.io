...
calculateBaseCharge()를 호출하는 곳을 모두 수정했다면, 이 함수를 enrichReading()안에 중첩시킬 수 있다.

여기서 주의점은 enrichReading()처럼 정보를 추가해 반환할 때 원본 측정값 레코드는 변경하지 않아도 한다는 것이다.
...
이제 applyShipping에 전달되는 다양한 매개변수를 살펴보자.
