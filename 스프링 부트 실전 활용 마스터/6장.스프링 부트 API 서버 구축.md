...
그런데 Item 관련 정보중에서는 Json에 포함되지 않았지만, 가치 있는 정보가 더 있다.
조회한 Item 정보 전체로 교체(put)하거나, 일부를 변경(patch)하거나, 삭제(delete)할 수 있는 링크를 함ㄲ ㅔ제공한다면 사용자가 쉽게 해당 작업을 수행할 수 있다.
이런 기능을 제공하기 위해 먼저 스프링 헤이티오스를 애플리케이션에 추가하자.
...
```
  ...
  spring-boot-starter-gateoas를 추가하면 스프링 헤이티오스를 사용할 수 있다.
  스프링 헤이티오스는 원래 스프링 mvc를 지원하는 용도로 만들어져서 스프링 MVC와 아파치 톰캣을 사용할 수 있게 해주는 spring-boot-starter-web이 포함돼 있다.
  
  
  이제 스프링 헤이티오스를 웹 컨트롤러에 적용하는 방법을 알아보자.
...
컨트롤러 도입부는 ApiItemController와 거의 동일하다.
  특별한 추가 조치를 하지 않아도 하이퍼미디어 기능을 적용할 수 있다.
